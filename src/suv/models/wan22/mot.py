...
class MoT(nn.Module):
    ...
    def _mixed_attention(
        ...
        # `torch.is_grad_enabled()` skips the checkpoint wrapper under no_grad (e.g.
        # the action-only video prefill) -- recompute-on-backward is pointless when
        # there is no backward. No effect on the normal grad-enabled training path.
