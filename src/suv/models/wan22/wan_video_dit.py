...
from .slot_layout import (
    SlotLayout,
    build_slot_causal_frame_mask,
    build_slot_text_context_mask,
    expand_frame_mask_to_tokens,
    tile_action_group_mask,
)
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, ctx_mask: Optional[torch.Tensor] = None, compatibility_mode=True):
    if compatibility_mode:
        ...
        # Opt-in FlexAttention for block-sparse self-attention masks (slot /
        # causal). Routes only square 2D bool masks; cross-attention masks and
        # the default dense path are untouched. See flex_attn.use_flex.
        from .flex_attn import flex_sdpa, use_flex

        if use_flex(ctx_mask):
            x = flex_sdpa(q, k, v, ctx_mask)
        else:
...
class WanVideoDiT(torch.nn.Module):
    ...
    def _validate_forward_inputs(
        ...
        if slot_layout is not None:
            if num_latent_frames != slot_layout.total_frames:
                raise ValueError(
                    "`latents` temporal dimension must match slot layout total_frames: "
                    f"got T={num_latent_frames}, expected {slot_layout.total_frames}."
                )
            if context.shape[1] % slot_layout.num_slots != 0:
                raise ValueError(
                    "`context` length must be divisible by slot count in slot mode: "
                    f"context_L={context.shape[1]}, num_slots={slot_layout.num_slots}."
                )
        if timestep.ndim != 1:
        ...
        if self.action_conditioned:
            ...
            if not allow_text_only_single_frame:
                ...
                action_groups = slot_layout.virtual_frames - 1 if slot_layout is not None else num_latent_frames - 1
                if action.shape[1] % action_groups != 0:
                    raise ValueError(
                        "action horizon must be divisible by the number of temporal action groups, "
                        f"got action_horizon={action.shape[1]}, groups={action_groups}"
                    )
    ...
    def build_video_to_video_mask(
        ...
        if slot_layout is not None:
            if self.video_attention_mask_mode not in {"first_frame_causal", "per_frame_causal"}:
                raise ValueError(
                    "slot_layout requires video_attention_mask_mode to be one of "
                    "['first_frame_causal', 'per_frame_causal'], "
                    f"got {self.video_attention_mask_mode!r}."
                )
            expected_seq_len = slot_layout.total_frames * video_tokens_per_frame
            if video_seq_len != expected_seq_len:
                raise ValueError(
                    "`video_seq_len` must equal slot_layout.total_frames * video_tokens_per_frame "
                    f"in slot mode, got {video_seq_len} vs {expected_seq_len}."
                )
            frame_mask = build_slot_causal_frame_mask(
                slot_layout,
                device=device,
                mask_mode=self.video_attention_mask_mode,
            )
            return expand_frame_mask_to_tokens(frame_mask, video_tokens_per_frame)
        ...
        if self.video_attention_mask_mode == "first_frame_causal":
            ...
            video_mask[:condition_tokens, condition_tokens:] = False
            return video_mask
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            condition_latent_frames = max(int(condition_latent_frames), 1)
            condition_latent_frames = min(condition_latent_frames, int(x.shape[2]))
            ...
            token_timesteps[:, :condition_latent_frames, :] = 0
        ...
        if self.action_conditioned and action is not None:
            ...
            # new mask
            num_temporal_groups = slot_layout.virtual_frames - 1 if slot_layout is not None else f - 1 # first latent frame do not attend to actions
            ...
            if slot_layout is not None:
                action_group_mask = tile_action_group_mask(
                    action_group_mask,
                    slot_layout,
                    tokens_per_frame,
                )
            ...
            if slot_layout is None:
                ...
                segment_len = context_len // slot_layout.num_slots
                final_context_mask[:, :, :context_len] = build_slot_text_context_mask(
                    slot_layout,
                    tokens_per_frame,
                    context_mask.reshape(batch_size, slot_layout.num_slots, segment_len),
                    condition_segment=0,
                )
            ...
            if slot_layout is None:
                ...
                segment_len = context_len // slot_layout.num_slots
                context_mask = build_slot_text_context_mask(
                    slot_layout,
                    tokens_per_frame,
                    context_mask.reshape(batch_size, slot_layout.num_slots, segment_len),
                    condition_segment=0,
        ...
        if slot_layout is not None:
            frame_freqs = self.freqs[0][slot_layout.virtual_frame_index(device=self.freqs[0].device)]
        freqs = torch.cat([
    ...
    def forward(
        ...
        self_attn_mask = self.build_video_to_video_mask(
            ...
            slot_layout=slot_layout,
        ) if slot_layout is not None or self.video_attention_mask_mode != "bidirectional" else None # special rule for faster speed
